...
public class Hand {
	...
	private static ArrayList<Hand> ExplodeHands(ArrayList<Hand> explodedHands) {
		...
		for (int iCard = 0; iCard < 5; iCard++) {
			ReturnHands = cardSubstitution(iCard, ReturnHands);
		}
		return ReturnHands;
	}

	public static ArrayList<Hand> getExplodeHands(Hand h) {
		return getExplodeHands(h);
	}

	private static ArrayList<Hand> cardSubstitution(int iCardSub, ArrayList<Hand> hands) {
		ArrayList<Hand> CreatedHands = new ArrayList<Hand>();
		Deck CreateDeck = new Deck(iCardSub);
